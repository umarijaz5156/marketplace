<div class="m-20">
    ...
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 6000)" id="alert-border-3"
    ...
    <div class="mb-4">
        ...
        <div>
                <h3 class="text-2xl font-bold text-gray-600 drk:text-white"><small
                    ...
                    <span class="{{$order->status == App\Enums\OrderStatus::InProgress->value ? 'bg-purple-200 text-purple-600' :
        ...
        py-2 px-16  rounded-full ">{{$order->status}}</span>
                </small></h3>
        </div>
        <h3 class="text-2xl font-bold text-gray-600 drk:text-white"><small
            class="ml-2 font-semibold text-gray-500 drk:text-gray-400">(Order # {{$order->id}})</small></h3>
        ...
        <div class="flex justify-between">
            <div>
                ...
                @if($isCancellationRequest)
                    @if($cancellationRequest->subject == 'Buyer')
                    <div class="hidden" id="loader3" role="status">
                    ...
                    <div id="cancellationButtons">
                        <p class="text-gray-600">Buyer Wants to cancel the order?</p>
            ...
            <tbody>
                <tr class="bg-white border-b drk:bg-gray-800 drk:border-gray-700">
                    <th rowspan="2" scope="row"
                        class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap drk:text-white">
                        <img src="{{asset('/gigs/images/'.$order->gig->mainImage->image_path) }}"
                            class="h-40 w-60 rounded-[4px]" alt="">
                        <p class="mt-[10px] font-medium text-base">{{$order->gig->gigDetail->title}}</p>
                    </th>
                    <td class="py-4 px-6">
                        <div class="flex">
                        {{$order->user->name}}
                        <livewire:forms.create-report :content="$order->user" contentType="{{App\Enums\ReportType::Buyer->value}}"/>
                        </div>
                    </td>
                    <td class="py-4 px-6">
    ...
    <div class="mt-14">
        <h3 class="text-2xl font-bold text-gray-600 drk:text-white"><small
                class="ml-2 font-semibold text-gray-500 drk:text-gray-400">Buyer Requirements</small></h3>
        @if($order->orderDetails->buyer_requirements)
        {{-- <textarea id="message" rows="4"
            ...
            readonly> --}}
        <p class="mt-2 block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 drk:bg-gray-700 drk:border-gray-600 drk:placeholder-gray-400 drk:text-white">{!!$order->orderDetails->buyer_requirements!!}</p>
        {{-- </textarea> --}}
        @endif

        @if(count($order->orderAttachments->where('is_delivery', false)) > 0)
        <div class="h-60  mt-4 overflow-y-auto relative">
    ...
    <div class="mt-10">
        <h3 class="text-2xl font-bold text-gray-600 drk:text-white"><small
                class="ml-2 font-semibold text-gray-500 drk:text-gray-400">Requests</small></h3>
    ...
    <div class="mt-10">
        <h3 class="text-2xl font-bold text-gray-600 drk:text-white"><small
                class="ml-2 font-semibold text-gray-500 drk:text-gray-400">Delivered Files</small></h3>
