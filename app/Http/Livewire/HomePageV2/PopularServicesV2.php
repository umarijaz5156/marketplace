...
namespace App\Http\Livewire\HomePageV2;

use App\Models\Seller\Gig;
use App\Models\Setting\PopularServicesHome;
use Livewire\Component;

class PopularServicesV2 extends Component
{
    ...
    public function render()
    {
        ...
        $query = Gig::join('gig_details as g_d', 'g_d.gig_id', '=', 'gigs.id')
                        ...
                        ->join('category_gig as c_g' , 'c_g.gig_id' ,'=' ,'gigs.id')
                        ->selectRaw('
                            gigs.id,
                            ...
                            c_g.category_id as category_id
                        ')
                        ->when($this->activeFilter == 2, function($query){
                            $query->where('category_id' , '=' , 2);

                        })
                        ->when($this->activeFilter == 3, function($query){
                        ...
                        ->groupBy('g_p.gig_id', 'c_g.category_id','users.is_online','gigs.id', 'g_d.slug', 'g_d.title', 'g_d.description', 'g_i.image_path', 'g_s.reviews_average', 'g_s.reviews_count', 's.seller_name', 'users.profile_photo_path')
                        ;
        ...
        if($popular_services_settings->status === 0){
            ...
            if(($data->count() < 4) && ($totalGigs->get()->count() > $data->count())){
                $dataExtra2 = $dataExtra->whereNotIn('gigs.id', $data->pluck('id'))
                                    ...
                                    ->orderBy('g_s.reviews_count','desc')
            ...
            if($this->activeFilter !== 1){
                // $data = $dataQuery->latest('gigs.created_at')->limit(6)->get();
                $data = $dataQuery->orderBy('g_s.reviews_count','desc')->limit(6)->get();
            } else{
                // $data = $dataQuery->latest('gigs.created_at')->limit(12)->get();
                $data = $dataQuery->orderBy('g_s.reviews_count','desc')->limit(12)->get();
        ...
        return view('livewire.home-page-v2.popular-services-v2',[
            'gigs' => $data->unique()
        ]);
    }
